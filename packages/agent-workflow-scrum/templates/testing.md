...
- Use explicit, verified Git bases when selecting outgoing checks: `{{RUNNER}} verify --base <verified-ref>`. Add semantic checks that path selection cannot infer.
- Before handoff run `{{RUNNER}} check` and `{{RUNNER}} docs` alongside relevant product checks.
