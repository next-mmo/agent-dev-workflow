...
1. Start non-trivial work with `{{RUNNER}} context -- "<scope>"`. Inspect current code, Git state, the active task, and affected PRD. Generated context and provider output are advisory.
...
4. Verify the real entry path, relevant negative cases, and configured checks. For outgoing committed scope, verify the base explicitly and use `{{RUNNER}} verify --base <verified-ref>`; never guess the base.
5. Review against acceptance criteria and run `{{RUNNER}} check`. Record fresh evidence, skipped checks, remaining risks, and human decisions before handoff. Passing checks do not constitute human acceptance.
