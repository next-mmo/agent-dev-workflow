...
const projectDocDefinitions = [
  ...
  ["Model recommendations", "docs/model-recommend.md"],
  ["Suggestions guide", "docs/suggestions/README.md"],
];
...
function resolveDocLink(href, docBySlug) {
  const external = /^[a-z][a-z0-9+.-]*:/i.test(href) || href.startsWith("//") || href.startsWith("#");
  if (external) return href;
  const [filePath] = href.split("#");
  const clean = filePath.replace(/^\.\//, "");
  if (clean.endsWith(".md")) {
    const slug = slugForPath(clean);
    if (docBySlug && docBySlug[slug]) {
      return `#doc-${slug}`;
    }
  }
...
function renderInline(text, docBySlug) {
  const code = [];
  const links = [];
  ...
  output = output.replace(/\u0000L(\d+)\u0000/g, (_match, index) => {
    ...
    const target = resolveDocLink(href, docBySlug);
...
function renderList(lines, startIndex, docBySlug) {
  ...
  while (next < lines.length) {
    ...
    if (bodyLines.length === 0 || bodyLines.every((line) => line.trim() === "")) {
      items.push(`<li>${renderInline(itemText, docBySlug)}</li>`);
    } else {
      const stripped = bodyLines.map((line) =>
        line.trim() === "" ? "" : stripLeadingSpaces(line, indent),
      );
      const bodyHtml = renderBlocks(stripped, docBySlug);
      items.push(`<li>${renderInline(itemText, docBySlug)}${bodyHtml}</li>`);
    }
...
function renderTable(rows, docBySlug) {
  ...
  const headCells = header
    .map((cell) => `<th>${renderInline(cell, docBySlug)}</th>`)
    .join("");
  const bodyRows = body
    .map((cells) => `<tr>${cells.map((cell) => `<td>${renderInline(cell, docBySlug)}</td>`).join("")}</tr>`)
    .join("");
  return `<table><thead><tr>${headCells}</tr></thead><tbody>${bodyRows}</tbody></table>`;
}

function renderBlocks(lines, docBySlug) {
  let html = "";
  let index = 0;
  while (index < lines.length) {
    const block = renderBlock(lines, index, docBySlug);
    html += block.html;
    index = block.next;
  }
  return html;
...
function renderBlock(lines, index, docBySlug) {
  ...
  if (heading) {
    ...
    return { html: `<h${level}>${renderInline(heading[2], docBySlug)}</h${level}>`, next: index + 1 };
  }
  ...
  if (trimmed.startsWith(">")) {
    ...
    return { html: `<blockquote>${renderBlocks(quote, docBySlug)}</blockquote>`, next };
  }
  ...
  if (trimmed.startsWith("|")) {
    ...
    return { html: renderTable(rows, docBySlug), next };
  }

  if (/^(\s*)([-*+]|\d+[.)])\s+/.test(trimmed)) {
    return renderList(lines, index, docBySlug);
  }
  ...
  return { html: `<p>${renderInline(paragraph.join(" "), docBySlug)}</p>`, next };
}

function renderMarkdown(markdown, docBySlug) {
  if (!markdown) return "<p>No content is available.</p>";
  const lines = markdown.replace(/\r\n?/g, "\n").split("\n");
  return renderBlocks(lines, docBySlug);
}
...
async function collectDocuments() {
  ...
  const prdPaths = (await markdownFiles("docs/prd")).filter(
  ...
  const activeTasks = (await markdownFiles("docs/tasks")).filter((filePath) =>
    /\/(?:todo|wip|blocked)-[^/]+\.md$/.test(`/${filePath}`),
  );
  const completedTasks = (await markdownFiles("docs/tasks/done")).filter((filePath) =>
    /\/done-[^/]+\.md$/.test(`/${filePath}`),
  );
  ...
  const suggestions = (await markdownFiles("docs/suggestions")).filter(
  ...
  for (const document of documents) {
    ...
    document.html = source ? renderMarkdown(source, docBySlug) : "";
  }
