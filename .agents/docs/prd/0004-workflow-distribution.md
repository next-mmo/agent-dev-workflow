...
> Consumer document stores: `.agents/docs/tasks/todo-0037-0004-consumer-document-stores.md`
...
9. Planning, solution, memory, evidence, and document-report commands work after initialization, using package defaults with optional consumer template overrides.
...
15. Initialize `evidence/`, `memory/`, `plans/`, `proposals/`, `solutions/`, and `tasks/` stores plus project-owned workflow, documentation instructions, architecture, defensive patterns, development, testing, and budget files. Exclude model recommendations; preserve existing files and legacy decisions. Doctor reports missing scaffold documents.
...
- [ ] Fresh consumer initialization creates README-backed evidence, memory, plans, proposals, solutions, and task stores while excluding model recommendations.
