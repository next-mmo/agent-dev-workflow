...
export async function buildVerificationPlan({ root = process.cwd(), base, head = "HEAD" } = {}) {
  ...
  if (skillsTouched && await exists(path.join(scope.repositoryRoot, "scripts/skill.sh"))) {
    addCheck(checks, "skill-adapters", "scripts/skill.sh check", "Canonical skill or generated adapter behavior changed.");
  }
  ...
  if ((productTouched || testTouched || buildTouched || ciTouched) && scripts.test) {
    addCheck(checks, "product-tests", "npm test", "Product, tests, build inputs, or CI execution changed.");
  }

  if ((productTouched || buildTouched) && scripts.build) {
    addCheck(checks, "build", "npm run build", "Runtime source or build/dependency inputs changed.");
  }
