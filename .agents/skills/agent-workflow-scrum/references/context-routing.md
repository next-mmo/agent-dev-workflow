...
npm run context -- "review auth" --base origin/main --level 1
...
2. Run `npm run change:scope -- --base <verified-ref>`.
3. Pass the same `--base <verified-ref>` to `npm run context` or `npm run verify:plan`.
